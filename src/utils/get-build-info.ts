...
export const autoGetHtml = (version: string, devFilePath: string) => {
    // 传递config.json的version字段，则自动在输出位置增加@version匹配。否则忽略@version
    let html: any = {
        keys: [],
        jsEntry: {},
    ...
    let getJsHtml = (dir: string) => {
        ...
        fs.readdirSync(dir).forEach((file: string) => {
            ...
            if (fs.statSync(pathname).isDirectory()) {
                ...
                let relFileName: string = './src/' + pathname.split('/src/')[1]
                html.originList.push(relFileName)
                let v = version ? version + '/' : ''
                let relFileKey: string = 'src/' + pathname.split('/src/')[1].split('index.html')[0] + v + 'index'
...
const getBuildInfo = (version: string) => {
